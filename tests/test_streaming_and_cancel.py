...
async def test_cancel_mid_stream_promptly_and_no_delta_after_cancelled():
    ...
    async with running_server(backend) as srv, connected_client(srv) as (client, _hello):
        ...
        async def _read_trailing():
            async for ev in client.events():
                if ev.get("type") == "response.audio.delta" and ev["response_id"] == response_id:
                    post_cancel_deltas.append(ev)
...
async def test_inter_delta_gap_bound_no_burst_then_flush():
    ...
    async with running_server(backend) as srv, connected_client(srv) as (client, _hello):
        ...
        assert max_gap <= bound, (
            f"max inter-delta gap {max_gap:.3f}s exceeds bound {bound:.3f}s (buffer-then-flush?)"
        )
...
async def test_bare_cancel_on_idle_session_is_noop():
    ...
    async with running_server(backend) as srv, connected_client(srv) as (client, _hello):
        ...
        ev = await next_event(client, {"server.status", "response.cancelled", "error"}, timeout=2.0)
