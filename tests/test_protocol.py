...
async def test_hello_advertises_protocol_version_and_audio_contract():
    async with (
        running_server(ToneBackend(sample_rate=24000)) as srv,
        connected_client(srv) as (_client, hello),
    ):
...
async def test_hello_capabilities_shape():
    async with running_server(ToneBackend()) as srv, connected_client(srv) as (_client, hello):
