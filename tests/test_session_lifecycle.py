...
async def test_response_failed_carries_code_and_message_and_session_usable():
    ...
    async with (
        running_server(ToneBackend(raises=True, segment_delay_ms=0)) as srv,
        connected_client(srv) as (client, _hello),
    ):
...
async def test_session_close_drains_then_closes():
    ...
    async with running_server(backend) as srv, connected_client(srv) as (client, _hello):
        ...
        ev = await next_event(client, {"response.audio.done", P.EVT_SESSION_CLOSED}, timeout=3.0)
...
async def test_three_cancel_close_paths_are_distinct():
    ...
    async with (
        running_server(ToneBackend(segment_count=1, segment_ms=20, segment_delay_ms=0)) as srv,
        connected_client(srv) as (client, _hello),
    ):
        await client.cancel_session()
