...
- [x] Phase 5: More streaming backends — **5a `voxtral_tts` DONE** (PR #4 merged; branch `feature/tts-voxtral-backend`; streaming_interval locked 0.3 s; latency+concurrency smoke executed — see Findings → *Phase 5a measurements*) and **5b `pocket_tts` DONE** (branch `feature/tts-pocket-backend`; 6 mlx-gated + 14 new lean tests incl. the decision-#2 ref_audio/frames_after_eos negative guard; streaming_interval locked 0.3 s; CC-BY-4.0; smoke executed — see Findings → *Phase 5b measurements*). `dia` (formerly 5c) split to `20260625-feature-tts-dia-backend.md` — **now SHIPPED** (conducted 2026-06-30 on branch `feature/tts-dia-backend`; the contract "dia reserved — NOT shipped" port-table row above the marker is left as a frozen historical record per the *don't edit the reviewed contract* convention). (Per that same decision, the `#### Phase 5a/5b` contract checkboxes above the marker are left as-is; completion is tracked here in the workspace.) Phase 6 promoted and conducted — see Phase 6 entry below.
